...
public class CinemaRoomController {
    ...
    @GetMapping("/{id}/seats")
    public ResponseEntity<List<Seat>> getSeats(@PathVariable Long id) {
        return ResponseEntity.ok(roomService.getSeatsByRoomId(id));
    }

    // Lấy chi tiết phòng + ghế + số hàng/cột
    @GetMapping("/{id}")
    public ResponseEntity<?> getRoomWithSeats(@PathVariable Long id) {
        Optional<CinemaRoom> optionalRoom = roomRepo.findById(id);
        if (optionalRoom.isEmpty()) {
            return ResponseEntity.notFound().build();
        }

        CinemaRoom room = optionalRoom.get();
        List<Seat> seats = seatRepo.findByCinemaRoom(room);

        return ResponseEntity.ok(buildRoomDetailResponse(room, seats));
    }

    // Lấy thông tin 1 phòng (không kèm danh sách ghế)
    @GetMapping("/rooms/{id}")
    public ResponseEntity<CinemaRoom> getRoomById(@PathVariable Long id) {
        CinemaRoom room = roomService.getRoomById(id);
    ...
    public ResponseEntity<?> createRoom(@RequestBody RoomRequest request) {
        try {
            CinemaRoom room = roomService.createRoom(request);
            return ResponseEntity.ok(room);
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<?> updateRoom(@PathVariable Long id, @RequestBody RoomRequest request) {
        try {
            CinemaRoom room = roomService.updateRoom(id, request);
            return ResponseEntity.ok(room);
        } catch (RuntimeException e) {
            return ResponseEntity.notFound().build();
        }
    ...
    private Map<String, Object> buildRoomDetailResponse(CinemaRoom room, List<Seat> seats) {
        Map<String, Object> result = new HashMap<>();
        result.put("id", room.getCinemaRoomId());
        result.put("name", room.getCinemaRoomName());
        result.put("rows", calculateMaxRow(seats));
        result.put("cols", calculateMaxCol(seats));
    ...
    private int calculateMaxRow(List<Seat> seats) {
        return seats.stream()
                .mapToInt(seat -> {
                    String row = seat.getSeatRow().toUpperCase();
                    return (!row.isEmpty()) ? row.charAt(0) - 'A' : 0;
                })
                .max().orElse(0) + 1;
    }
    ...
    private int calculateMaxCol(List<Seat> seats) {
        return seats.stream()
                .mapToInt(seat -> {
                    try {
                        return Integer.parseInt(seat.getSeatCol());
                    } catch (NumberFormatException e) {
                        return 0;
                    }
                })
