...
// import { BadgePlus, X ,SquarePen} from 'lucide-react';
...
//           {isEdit ? <><SquarePen size={16} className="me-1 mt-1" />Cập nhật</> : <><BadgePlus size={25} className="me-1" />Thêm phim</>}
//         </Button>
...
export default function MovieForm({ movieId, onSuccess }) {
  ...
  const renderInput = (label, name, type = 'text') => (
    <Form.Group className="mb-3">
      <Form.Label className="form-label fw-semibold">{label}</Form.Label>
      <Form.Control
        name={name}
        type={type}
        value={form[name]}
  ...
  return (
    <Form onSubmit={handleSubmit} className="p-4 p-md-5 rounded-4 shadow bg-white" style={{ maxWidth: 1000, margin: 'auto' }}>
      <h3 className="mb-4 text-primary fw-bold">{isEdit ? 'Cập nhật phim' : 'Thêm phim mới'}</h3>
      <Row>
        <Col md={6}>
          {renderInput('Tên phim (VN)', 'nameVN')}
          {renderInput('Tên phim (EN)', 'nameEN')}
        ...
        <Col md={6}>
          ...
          <Form.Group className="mb-3">
            <Form.Label className="form-label">Poster</Form.Label>
            <Form.Control type="file" isInvalid={!!errors.smallImage} onChange={e => {
              const file = e.target.files[0];
              setSmallImage(file);
              if (file) setPreviewSmallImage(URL.createObjectURL(file));
          ...
          <Form.Group className="mb-3">
            <Form.Label className="form-label">Banner</Form.Label>
            <Form.Control type="file" isInvalid={!!errors.largeImage} onChange={e => {
              const file = e.target.files[0];
              setLargeImage(file);
              if (file) setPreviewLargeImage(URL.createObjectURL(file));
      ...
      <Form.Group className="mb-3">
        <Form.Label className="form-label">Nội dung phim</Form.Label>
        <Form.Control as="textarea" name="content" rows={4} value={form.content} onChange={handleChange} isInvalid={!!errors.content} />
        <Form.Control.Feedback type="invalid">{errors.content}</Form.Control.Feedback>
      </Form.Group>
      ...
      <Form.Group className="mb-4">
        ...
        <Dropdown>
          <Dropdown.Toggle variant="outline-light" className="w-100 text-start rounded-3">
            {form.typeIds.length > 0 ? form.typeIds.map(id => {
              const type = types.find(t => t.id === id);
              return (
                <Badge key={id} bg="primary" className="me-1 mb-1">
                  {type?.name || 'Unknown'}
                  <span className="ms-1" style={{ cursor: 'pointer' }} onClick={e => {
                    e.stopPropagation();
                    handleRemoveType(id);
                  }}>×</span>
                </Badge>
      ...
      <div className='d-flex gap-2'>
        <Button type="submit" variant="primary" className='rounded-pill px-4 py-2 d-flex align-items-center'>
          {isEdit ? <><SquarePen size={16} className="me-2" />Cập nhật</> : <><BadgePlus size={20} className="me-2" />Thêm phim</>}
        </Button>
        <Button type="button" variant="outline-secondary" className='rounded-pill px-4 py-2 d-flex align-items-center' onClick={() => navigate('/admin/movies')}>
