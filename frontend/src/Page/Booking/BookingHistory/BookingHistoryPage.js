...
import styles from './bookingHistoryPage.module.scss'; // SỬ DỤNG FILE SCSS MỚI
...
const BookingHistoryPage = () => {
    ...
    if (loading) {
        return <div className={cx('page-container', 'centered-message')}>Đang tải lịch sử đặt vé...</div>;
    }
    if (error) {
        return <div className={cx('page-container', 'centered-message', 'error-message')}>{error}</div>;
    }
    ...
    return (
        <div className={cx('page-container')}>
            ...
            {bookings.length === 0 ? (
                <div className={cx('centered-message')}>Bạn chưa có đơn đặt vé nào.</div>
            ) : (
