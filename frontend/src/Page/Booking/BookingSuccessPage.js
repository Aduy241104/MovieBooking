...
const BookingSuccessPage = () => {
        ...
        const fetchDetails = async () => {
            try {
                ...
                if (response.data && response.data.result) {
                    ...
                    console.warn(
                        "BookingSuccessPage - Could not fetch booking details for ID:",
                        bookingId,
                        "Response data:",
                        response.data
                    );
    ...
    return (
        <div className="container mt-5 text-center">
            ...
            <p>
                Mã đặt vé của bạn là: <strong>#{bookingDetails.bookingId}</strong>
            </p>

            <div className="card mt-4 mx-auto" style={{ maxWidth: "600px" }}>
                <div className="card-header fw-bold">
                    {" "}
                    {/* Thêm fw-bold cho tiêu đề card */}
                    Chi tiết vé
                </div>
                <div className="card-body text-start">
                    {" "}
                    {/* text-start để chữ căn trái */}
                    <p>
                        <strong>Phim:</strong> {bookingDetails.screening?.movieNameVn || "N/A"}
                    </p>
                    <p>
                        <strong>Suất chiếu:</strong> {formatScreeningDateTime(bookingDetails.screening?.showDateTime)}
                    </p>
                    <p>
                        <strong>Phòng chiếu:</strong> {bookingDetails.screening?.cinemaRoomName || "N/A"}
                    </p>
                    <p>
                        <strong>Ghế:</strong>{" "}
                        {bookingDetails.bookedSeats?.map((s) => `${s.seatRow}${s.seatCol}`).join(", ") || "N/A"}
                    </p>
                    <p>
                        <strong>Tổng tiền:</strong> {(bookingDetails.totalAmount || 0).toLocaleString("vi-VN")}đ
                    </p>
                    <p>
                        <strong>Trạng thái:</strong>{" "}
                        <span className={`badge bg-${bookingDetails.bookingStatus === "PAID" ? "success" : "warning"}`}>
                            {bookingDetails.bookingStatus || "N/A"}
                        </span>
                    </p>
                    {bookingDetails.paymentMethod?.methodName && (
                        <p>
                            <strong>Thanh toán:</strong> {bookingDetails.paymentMethod.methodName}
                        </p>
                    )}
                </div>
            </div>
            ...
            <div className="mt-4 d-flex justify-content-center gap-3">
                {" "}
                {/* d-flex và gap cho nút */}
                <Link to="/booking/history">
                    <CustomizeButton secondary large>
                        {" "}
                        {/* Bỏ me-3 nếu dùng gap */}
                        Xem Lịch Sử Đặt Vé
                    </CustomizeButton>
                </Link>
                <Link to="/">
